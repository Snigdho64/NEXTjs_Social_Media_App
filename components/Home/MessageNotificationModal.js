...
export default function MessageNotificationModal({
  ...
  const onModalClose = () => setShowMsgModal(false);
  ...
  const handleSubmit = (e) => {
    ...
    if (socket.current) {
      socket.current.emit('sendMsgFromNotification', {
        senderId: user._id,
        receiverId: receivedMessage.sender,
        msg: text,
      });
  ...
  return (
    <Modal
      ...
      closeOnDimmerClick
    >
      <Modal.Header content={`New Messsage  from ${receivedMessage.name}`} />
      <Modal.Content>
        <div className="bubbleWrapper">
          <div className="inlineContainer">
            <img className="inlineIcon" src={receivedMessage.profilePicUrl} />
          </div>

          <div className="otherBubble other">{receivedMessage.msg}</div>

          <span className="other">{calculateTime(receivedMessage.date)}</span>
        </div>
        ...
        <div style={{ marginTop: '5px' }}>
          <Link href={`/messages?message=${receivedMessage.sender}`}>
